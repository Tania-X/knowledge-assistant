...
class SearchService:
    ...
    def _vector_search(self, query: str, top_k: int) -> List[Dict]:
        ...
        for doc_id, source_type, chunk_index, distance in results:
            if source_type == 'web':
                cursor.execute(
            ...
            row = cursor.fetchone()
            if row:
                # 如果文档ID已经存在，只保留距离更近的结果
                if doc_id not in doc_map or distance < doc_map[doc_id]['distance']:
                    doc_map[doc_id] = {
                        'id': doc_id,
                        'type': source_type,
                        'source': row[0],
        ...
        # 转换为列表并按相似度排序
        search_results = sorted(doc_map.values(), key=lambda x: x['similarity'], reverse=True)
