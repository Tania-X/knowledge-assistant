...
- ✅ **分块嵌入策略**：支持智能分块、语义分块和固定分块三种策略，提升长文档检索精度
- ✅ **向量搜索**：基于sentence-transformers和FAISS的语义搜索，支持分块检索结果聚合
- ✅ **智能问答**：结合本地大模型，基于知识库回答问题
- ✅ **索引管理**：支持索引重建和清空，实时查看索引统计信息
...
#### 2. 文件索引
- 选择单个文件或整个目录进行索引
- 支持增量索引（文件夹级/文件级），只处理新增或修改的文件
- 自动识别文件类型，提取文本内容
- 采用分块策略处理长文档，生成多个向量嵌入并存储
- 实时显示索引统计信息

#### 3. 索引管理
- 查看索引统计：实时显示文件索引、网页内容和FAISS索引数量
- 重建索引：从数据库重新加载所有嵌入向量，修复索引不一致问题
- 清空索引：删除所有FAISS索引数据和数据库中的嵌入向量
...
search:
  max_results: 5
  max_context_length: 2000                           # 上下文最大长度
```
