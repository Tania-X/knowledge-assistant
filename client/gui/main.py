...
class KnowledgeBaseClient:
    ...
    def setup_file_tab(self):
        ...
        ttk.Button(file_frame, text="增量索引", command=self.incremental_index).pack(side=tk.LEFT, padx=5)
    ...
    def index_file(self, file_path: str):
        ...
        def do_index():
            ...
            try:
                ...
                if result['status'] == 'success':
                    doc_id = self.indexer.save_to_db(result)
                    ...
                    self.file_result.insert(tk.END, f"文档ID: {doc_id}\n\n")
    ...
    def index_directory(self, dir_path: str):
        ...
        def do_index():
            ...
            try:
                ...
                for result in results:
                    if result['status'] == 'success':
                        self.indexer.save_to_db(result)
                        success_count += 1
                
                self.file_result.insert(tk.END, f"✓ 完成，成功索引 {success_count}/{len(results)} 个文件\n\n")
    ...
    def incremental_index(self):
        ...
        def do_incremental():
            ...
            self.file_result.insert(tk.END, f"正在增量索引: {dir_path}...\n")
            self.file_result.update()
            
            try:
                result = self.indexer.index_directory_incremental(dir_path)
